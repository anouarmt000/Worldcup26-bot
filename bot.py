...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Watch Match
        ...
        watch_btns = InlineKeyboardMarkup([
            [InlineKeyboardButton("🟢 Yalla Shoot يلا شوت", url="https://yallashoot.one")],
            [InlineKeyboardButton("🔵 Kora Live كورة لايف", url="https://koora-live.tv")],
            [InlineKeyboardButton("🟣 Kora Star كورة ستار", url="https://koora-star.com")],
...
async def auto_cache_warmer(app):
    """يحمّي الكاش تلقائياً — يراعي الـ TTL ولا يستنزف الـ API"""
    await asyncio.sleep(30)
    logger.info("🔥 Auto cache warmer started")

    # أول تحميل عند البداية فقط
    try:
        logger.info("🔄 Initial warm-up: standings + scorers")
        await get_standings()
        await asyncio.sleep(2)
        await get_top_scorers()
    except Exception as e:
        logger.error(f"Initial warm-up error: {e}")

    while True:
        # انتظر TTL_STANDINGS (6 ساعات) قبل تجديد الترتيب والهدافين
        await asyncio.sleep(TTL_STANDINGS)
        try:
...
def main():
    ...
    logger.info("⚽ World Cup 2026 Pro Bot v7.0 — Watch + Stars EDITION — LIVE!")
