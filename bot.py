...
# ── Live Matches (كاش 3 دقائق) — كأس العالم فقط ──────────
...
# ── Recent Results — نظام هجين: ثابت + API ───────────────
# النتائج الثابتة تُحدَّث يدوياً هنا بعد كل مباراة
# أما API فيُستخدم كمصدر ثانوي إذا كانت المباراة حية انتهت للتو
STATIC_RESULTS = [
    # الشكل: date, home, away, home_goal, away_goal, group
    # ── المرحلة الأولى — يُضاف هنا بعد كل مباراة ──
    {"date":"2026-06-11","home":"🇲🇽 Mexico","away":"🇿🇦 South Africa","hg":0,"ag":0,"group":"A","done":False},
]

async def get_recent_results() -> list:
    cached = CACHE.get("recent_results")
    if cached is not None:
        logger.info("💾 Cache HIT: recent_results")
        return cached

    results = []

    # 1) حاول جلب النتائج من API أولاً
    ...
    # 2) إذا API فارغ، استخدم النتائج الثابتة المحدَّثة يدوياً
    if not results:
        logger.info("ℹ️ API empty — using static results")
    ...
    for d in sorted(by_date.keys(), reverse=True)[:7]:
        ...
        for m in by_date[d]:
            ...
            if hg > ag:
                ...
                home_icon = away_icon = "🤝"
            src = " _[live]_" if m.get("source") == "api" else ""
            lines.append(
                f"⚽ {m['home']}{home_icon} *{hg} — {ag}* {away_icon}{m['away']}\n"
                f"   {grp_lbl}: {m['group']}{src}"
            )
    return "\n".join(lines)
...
async def result_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # امسح الكاش لإظهار النتيجة فوراً
        CACHE.force_expire("recent_results")
